...
export const metadata = { title: "Weekly Jams" };
    ...
    .filter(
      (p) =>
        p.playlist.extension?.[JSPF_PLAYLIST_KEY]?.additional_metadata
          ?.algorithm_metadata?.source_patch === algorithm,
    )
...
export default async function WeeklyJamsPage() {
    ...
    <PageShell className="pt-8">
      ...
      >
        <Body
          ...
          emptyMessage="ListenBrainz hasn't generated a Weekly Jams playlist for you yet — your first one usually shows up after a couple weeks of listening."
        />
      </Suspense>
    </PageShell>
  );
